        ...
        <div className={styles.heroGrid}>
          ...
          <div className={`${styles.audit} ${styles.heroPrint}`} style={dvar(0.25)} role="group" aria-label="Sample ledger — flip a switch to walk the real cancel flow">
            ...
            {SAMPLE_SUBS.map((s, i) => {
              ...
              return (
                <Fragment key={s.n}>
                  <div className={`${styles.auditRow} ${styles.heroPrint} ${off ? styles.auditRowOff : ""} ${ph === "verified" ? styles.vflash : ""}`} style={dvar(rowDelays[i])}>
                  ...
                  {flows[s.n] ? <FlowLog lines={CANCEL_FLOWS[s.n]} shown={flows[s.n].shown} closing={flows[s.n].closing} /> : null}
                </Fragment>
              );
            })}
