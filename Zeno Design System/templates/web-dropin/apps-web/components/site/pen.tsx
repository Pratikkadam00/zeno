...
export function RunningTally() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      chip.classList.toggle(styles.penTallyShow, hero.getBoundingClientRect().bottom < 64);
      const max = document.documentElement.scrollHeight - window.innerHeight;
      const p = max > 0 ? Math.min(1, window.scrollY / max) : 0;
      ...
      if (n !== shown) {
        ...
        shown = n;
        tween(sumFirst(n));
        replay(chip, styles.penTallyTick);
        if (grew && n > 0) {
          const isOff = off[n - 1];
          floatEl.textContent = isOff ? "CANCELLED · $0.00" : `+ $${SAMPLE_SUBS[n - 1].amt.toFixed(2)}`;
          floatEl.classList.toggle(styles.ptFloatOk, isOff);
          replay(floatEl, styles.ptFloatGo);
        }
      }
    };
