...
## Phase D — "The Honest Ledger" design port (added 2026-07-10)

The design system was updated (uncommitted as of this note): committed direction **The Honest Ledger** (v2) — 4 named signatures (Ledger Line, Zeno Stamp, Column Heads, Print-in), motion system "everything settles like paper" with an explicit Reanimated-4 + expo-haptics mapping in `tokens/motion.css`, navy-cast ink tokens on warm paper `#FAF9F5` with `--rule`/`--ink-panel`/`--stamp-*` signature tokens, art-directed dark mode, ink-primary buttons (green reserved for money-positive/verified), truthfulness rails preserved. All 14 ui_kit screens updated + NEW `CoachScreen`, `FamilyScreen`, `Ledger.jsx` kit (LedgerLine/SectionHead/ColumnHeads/Stamp/TearEdge/SkeletonRow/BottomSheetLite/CodeBoxes), new icon/wordmark assets (3 icon candidates), App Store screenshot kit.

**Order: P5 first** (design-independent logic tests = the safety net under the big UI port; component-test INFRA lands in P5.1 but screen-level component tests are written per-D-phase as screens land, never against the old UI).

- **D0 Foundation** — commit the DS update as its own commit (source of truth versioned first). Port tokens to `src/theme/tokens.ts` (ink scale, paper, rule/rule-strong/ink-panel/stamp colors, border remaps, art-directed dark set); VERIFY the RN font trio matches the DS trio (Space Grotesk / Hanken Grotesk / JetBrains Mono) — verify, don't assume; new `src/theme/motion.ts` (durations, 45ms stagger, `settle` d22/s260, `thunk` d14/s420, reduced-motion via AccessibilityInfo) + `src/theme/haptics.ts` map; install `expo-haptics`, `@gorhom/bottom-sheet`. OWNER DECISION: app icon (green/navy/paper candidates in `assets/app-icons/`) + splash background; wire chosen assets in app.config.ts. First emulator eyeball (tokens shift colors app-wide).
- **D1 Core kit** — update Card (hairline `--rule`, less chrome), Button (ink primary), Badge, ListRow per DS diffs; NEW RN components: LedgerLine, SectionHead/ColumnHeads, Stamp (thunk spring + Success haptic), TearEdge/SheetShell, SkeletonRow (Reanimated shimmer), BottomSheet (gorhom wrapper: option-list + confirm-destructive variants), CodeBoxes, print-in stagger helper (FlatList-safe, transform/opacity only). Smoke tests on the new components via P5.1 infra.
- **D2 Hot screens** — Chrome/tab bar, Dashboard, Subscriptions (PRESERVE the P4 FlatList/memo/debounce work), SubscriptionDetail, AddSubscription.
- **D3 Flows** — Discover (scanline + tear-edge receipt results), CancelFlow + the Stamp verified-cancel celebration (the app's ONE celebration — no confetti anywhere), Paywall, Onboarding + splash handoff, Calendar, Insights, Budget, BudgetRecap, Settings (Alert.alert pickers → BottomSheet; closes the P4 debt). Emulator drive per flow (local `expo run:android`; EAS quota may have reset July 1 — confirm before any cloud build).
- **D4 New screens + legacy-kit retirement** — port Coach + Family from the new mockups; this migrates the last screens off legacy `Surface`/`PrimaryButton` (closes the deferred P4.2 consolidation). Consent + disclaimer copy preserved VERBATIM.
- **D5 Gap closure + final verification** — 6 screens have no mockups yet (Wrapped, Login, Security/App lock, Notifications inbox, Spend Twin, Widgets). DECISION: run the design session once more scoped to exactly these 6 (reusing the committed signatures) rather than improvising in-app — the DS stays the single source of truth; they inherit D0 tokens in the interim so nothing looks broken. Then: full emulator pass, dark-mode pass, reduced-motion pass, a11y re-check, coverage ratchet (P5.5), store screenshots from `app_store/` (owner asset).

Standing rails for every D-phase: truthfulness copy rules (§14 / DS readme "truthfulness rails") are non-negotiable; P4 perf work (notification diff pipeline, virtualization, memoization) must survive the port; per-phase commits with `typecheck && lint && vitest` green.
