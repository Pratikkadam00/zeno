...
export function PenHead({ children, center = false, style }: { children: ReactNode; center?: boolean; style?: CSSProperties | undefined }) {
...
export function MaskLines({ lines, delay = 0, className, style }: { lines: ReactNode[]; delay?: number; className?: string | undefined; style?: CSSProperties | undefined }) {
  const ref = useRef<HTMLHeadingElement>(null);
  const inView = useInView(ref, { once: true, margin: "-56px" });
  const reduce = useReducedMotion();
  const on = inView || !!reduce;
...
export function DrawBar({ color = "var(--stamp-alert)", top = 2, bottom = 2, delay = 0, style }: { color?: string; top?: number; bottom?: number; delay?: number; style?: CSSProperties | undefined }) {
  const reduce = useReducedMotion();
  return (
    <m.span
      aria-hidden="true"
...
export function Odometer({ value, className, style }: { value: number | string; className?: string | undefined; style?: CSSProperties | undefined }) {
  ...
  useEffect(() => {
    ...
    // Deliberate: entering view kicks off the roll animation phase; the flag
    // must flip synchronously here, not derive from render.
