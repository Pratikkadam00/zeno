...
const guideOverrides: Record<string, string[]> = {
  netflix: [
  ...
  return rows
    ...
    .map((row) => {
      ...
      const input: ServiceInput = {
        ...
        website,
        cancelUrl,
        cancelDifficulty: cancelDifficulty as CancellationDifficulty,
...
function defineServices(inputs: readonly ServiceInput[]): Service[] {
...
export const services: Service[] = uniqueBySlug([
  ...defineServices(parseRequestedRows(requestedRows)),
  ...defineServices(parseExpansionRows(expansionRows))
]);

const categoryMap: Record<ServiceCategory, SubscriptionCategory> = {
...
export function toSubscriptionCategory(category: ServiceCategory): SubscriptionCategory {
...
export function toServiceRecord(service: Service): ServiceRecord {
  let supportContact: ServiceRecord["supportContact"];
...
export const serviceRecords: ServiceRecord[] = services.map(toServiceRecord);
...
export function getPopularServices(): Service[] {
  ...
  return popularIds.map((id) => services.find((service) => service.id === id)).filter(Boolean) as Service[];
}
...
export function searchServiceRecords(query: string, limit = 10): ServiceRecord[] {
