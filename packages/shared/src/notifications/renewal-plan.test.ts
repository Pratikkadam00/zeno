...
describe("renewal reminder plan", () => {
  ...
  describe("quiet hours that wrap midnight", () => {
    ...
    it("pushes a 23:00 UTC trigger to 06:00 the next day", () => {
      const triggerAt = planDayOfTrigger(new Date(Date.UTC(2026, 5, 20, 23, 0, 0)));
      expect(triggerAt).toBe(new Date(Date.UTC(2026, 5, 21, 6, 0, 0, 0)).toISOString());
    });

    it("pushes a 05:00 UTC trigger to 06:00 the same day", () => {
